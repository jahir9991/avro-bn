  ...
  isExact(needle, heystack, start, end, not) {
    return (
      ((start >= 0 &&
        ...
        heystack.substring(start, end) === needle) as any) ^ not
    );
  },
  isCaseSensitive(c) {
...
export class NgAvroBnDirective {
  @Input() avroFormField: any;

  constructor(private element: ElementRef) {}
  @HostListener('keydown', ['$event'])
  onKeydown(event: KeyboardEvent) {
    if (event.key === ' ' || event.key === 'Enter' || event.key === 'Tab') {
      this.element.nativeElement.value = bntry.parse(
        this.element.nativeElement.value
      );
    }
    if (this.avroFormField) {
      this.avroFormField.setValue(this.element.nativeElement.value);
    }
  }
  @HostListener('blur')
  onBlur() {
